...
| tests/test_os_globe_high_resolution.py | 普通海域、任务领取后的普通海域、隐蔽海域及颜色保护的 4 项高分辨率回归测试 |
...
当前预期是分辨率 10 项、高分辨率大世界识别 4 项、G.M. 指令页识别 3 项、更新器 10 项，共 27 项通过。
...
真实故障截图证明坐标换算和点击都正确。问题发生在识别阶段：2560×1440 截图缩小到 1280×720 后，海域标签文字的抗锯齿和平均颜色会发生变化。2026-07-17 的 `ZONE_OBSCURE` 相似度约为 0.8146，2026-07-18 的 `ZONE_DANGEROUS` 相似度约为 0.8132，均低于原来的 0.85 门槛。任务领取后的另一张 `ZONE_DANGEROUS` 画面相似度约为 0.8126，颜色容差约为 18.2，超过最初采用的 10。

修复统一作用于 `ZONE_TYPES` 中的六种海域：

- 模板相似度门槛改为 0.80。
- 同时要求海域标签颜色在阈值 20 内匹配，避免仅靠放宽模板造成误识别，同时覆盖任务领取后的真实颜色波动。
- 只修改大世界海域类型检测；全局模板门槛和所有点击坐标保持不变。
- 回归测试使用两种普通海域画面与隐蔽海域的真实故障截图最小区域，并交叉验证不同类型不会互相误认。
- 离线扫描 108 张历史错误截图时，颜色阈值 20 只补回 3 张真实普通海域画面，没有出现多个海域类型同时命中的情况。
...
7. 运行 27 项自动测试、py_compile 和必要的模拟器冒烟测试。
