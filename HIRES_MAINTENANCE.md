...
| tests/test_os_globe_high_resolution.py | 普通海域、任务领取后的普通海域、三种红色警告普通海域、卡利比安海 D、隐蔽海域及颜色保护的 8 项高分辨率回归测试 |
...
| tests/fixtures/zone_dangerous_mission_checkout_2560x1440_downsampled.png | 从任务领取后停留画面提取的普通海域最小识别区域 |
| tests/fixtures/zone_dangerous_warning_2560x1440_downsampled.png | 从红色警告海域停留画面提取的普通海域最小识别区域 |
| tests/fixtures/zone_dangerous_caribbean_d_2560x1440_downsampled.png | 从卡利比安海 D 停留画面提取的普通海域最小识别区域 |
| tests/fixtures/zone_dangerous_warning_northwest_f_2560x1440_downsampled.png | 从西北航道 F 红色警告画面提取的普通海域最小识别区域 |
| tests/fixtures/zone_dangerous_warning_forward_base_2560x1440_downsampled.png | 从核心区前哨站红紫色警告画面提取的普通海域最小识别区域 |
...
当前预期是分辨率 10 项、高分辨率大世界识别 8 项、G.M. 指令页识别 3 项、更新器 10 项，共 31 项通过。
...
真实故障截图证明坐标换算和点击都正确。问题发生在识别阶段：2560×1440 截图缩小到 1280×720 后，海域标签文字的抗锯齿和平均颜色会发生变化。2026-07-17 的 `ZONE_OBSCURE` 相似度约为 0.8146，2026-07-18 的 `ZONE_DANGEROUS` 相似度约为 0.8132，均低于原来的 0.85 门槛。任务领取后的另一张 `ZONE_DANGEROUS` 画面相似度约为 0.8126，颜色容差约为 18.2。2026-07-21 的红色警告样式相似度约为 0.8141，颜色容差约为 24.25。2026-07-22 的卡利比安海 D 画面相似度约为 0.7943，低于上一版 0.80 门槛，但颜色容差约为 17.65，仍符合普通海域标题颜色。2026-07-28 的西北航道 F 红色警告样式相似度约为 0.8129，模板匹配正常，但颜色容差约为 31.04，刚好超过当时门槛 30。2026-08-04 的核心区前哨站红紫色警告样式相似度约为 0.8122，模板匹配同样正常，但颜色容差约为 38.96，超过上一版只对普通海域使用的门槛 35。

修复统一作用于 `ZONE_TYPES` 中的六种海域：

- 局部模板相似度门槛改为 0.70；该值只用于已选海域标题，不改变项目的全局识别门槛。
- 同时要求海域标签颜色匹配。五种非普通海域继续使用默认阈值 30；只有 `ZONE_DANGEROUS` 使用 40，以覆盖普通、红色和红紫色警告样式的真实颜色波动，同时避免扩大其他海域类型的识别范围。
- 只修改大世界海域类型检测；全局模板门槛和所有点击坐标保持不变。
- 回归测试使用六种普通海域画面与隐蔽海域的真实故障截图最小区域，并交叉验证不同类型不会互相误认。
- 离线扫描 147 张历史错误截图时，颜色校验通过的真实海域最低相似度是 0.7943，颜色校验通过的非目标候选最高相似度是 0.5720；0.70 位于两者之间并保留超过 0.12 的观察裕量。把 `ZONE_DANGEROUS` 的颜色阈值从 35 调到 40 只补回 9 张核心区前哨站相关真实故障截图，没有出现多个海域类型同时命中的情况；继续调到 45、50 或忽略颜色都不会新增历史命中，因此 40 已覆盖所有已观察到的强模板匹配普通海域。
...
7. 运行 31 项自动测试、py_compile 和必要的模拟器冒烟测试。
