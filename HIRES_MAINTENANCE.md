...
| tests/test_os_globe_high_resolution.py | 普通海域、任务领取后的普通海域、红色警告普通海域、隐蔽海域及颜色保护的 5 项高分辨率回归测试 |
...
当前预期是分辨率 10 项、高分辨率大世界识别 5 项、G.M. 指令页识别 3 项、更新器 10 项，共 28 项通过。
...
真实故障截图证明坐标换算和点击都正确。问题发生在识别阶段：2560×1440 截图缩小到 1280×720 后，海域标签文字的抗锯齿和平均颜色会发生变化。2026-07-17 的 `ZONE_OBSCURE` 相似度约为 0.8146，2026-07-18 的 `ZONE_DANGEROUS` 相似度约为 0.8132，均低于原来的 0.85 门槛。任务领取后的另一张 `ZONE_DANGEROUS` 画面相似度约为 0.8126，颜色容差约为 18.2。2026-07-21 的红色警告样式相似度约为 0.8141，颜色容差约为 24.25。
...
- 回归测试使用三种普通海域画面与隐蔽海域的真实故障截图最小区域，并交叉验证不同类型不会互相误认。
- 离线扫描 114 张历史错误截图时，颜色阈值 30 相比阈值 20 只补回 5 张真实普通海域画面，没有出现多个海域类型同时命中的情况。
...
7. 运行 28 项自动测试、py_compile 和必要的模拟器冒烟测试。
